...
export function BottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-200 flex items-center justify-around px-2 z-40 pb-safe">
      ...
      <Link href="/settings" className={cn("flex flex-col items-center justify-center w-16 h-full gap-1", pathname.startsWith("/settings") ? "text-blue-600" : "text-slate-400 hover:text-slate-600")}>
        <MoreHorizontal className={cn("w-5 h-5", pathname.startsWith("/settings") && "fill-blue-50")} />
